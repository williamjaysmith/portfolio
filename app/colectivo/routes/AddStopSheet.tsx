...
export function AddStopSheet({ route, currentOrder, onAdd, onClose }: AddStopSheetProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-md bg-white border border-[#2c2c2c] rounded-xl max-h-[80dvh] overflow-y-auto">
        <div className="flex items-center justify-between p-4 bg-[#2c2c2c]">
          <h2 className="font-black text-white">Add a stop to {route.label}</h2>
        ...
        <div className="p-4">
          ...
          {groups.map((group) => (
            <div key={group.label} className="mb-4">
              <h3
                className="text-xs font-bold uppercase tracking-wide mb-1 text-[#2c2c2c]"
                style={{ WebkitTextStroke: "0.7px #2c2c2c" }}
              >
                {group.label}
              </h3>
