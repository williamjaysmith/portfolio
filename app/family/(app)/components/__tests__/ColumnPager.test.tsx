...
describe("useColumnPage: which columns are on screen", () => {
  ...
  /**
   * 012 added an `openOn` option here so the Meals grid could open on today's
   * column, and 013 removed it again — its window now begins on today, so there
   * is no paged-away today to correct. The four tests that covered it went with
   * it rather than being left to assert a parameter nobody passes.
   *
   * The rule those tests sat beside is the one that matters and stays: **one
   * column per swipe** (FR-396), pinned above, for the Profile columns on Tasks,
   * Lists and Rewards. That rule is why Meals needed its own navigation instead
   * of a mode on this hook.
   */
