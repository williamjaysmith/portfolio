...
/**
 * Press-and-hold to reorder — the one gesture every board's list shares
 * (003 T076: FR-309, FR-310, FR-397, R321; 004 the Rewards board's columns;
 * 005 T005/R507: the Lists tab's items, its third consumer, which is why the
 * machine lives here and not beside any one board).
 *
 * One binding serves every list, because each is the same gesture over the
 * same pure reducer and they differ only in what they are a list OF: the
 * profile columns run across a board, a section's routines run down a column,
 * a list's items run down a card. Everything that differs is an argument — the
 * axis, which element a press may begin on, and what a drop commits to.
 *
 * **Press and hold, then move.** The reference specifies exactly that gesture
 * for its lists, and it is deliberately NOT `lib/family/drag-state.ts`: that
 ...
 *
 * **The keyboard reaches the same reducer** (FR-397) when `keyboard` is on.
 * Enter or Space on a handle picks the row up, the arrow keys along the list's
 * own axis move it, Enter or Space drops it and Escape puts it back — every
 * step announced through `announcement`, which the caller renders in an
 * `aria-live` region. A keyboard move is not a second implementation of
 * anything: it produces the same two indices the pointer does and hands them
 * to `reorderList`.
 *
 ...
 * container in the same order as `items`, which is exactly how every caller
 * renders them. A row that matches `rowSelector` but not `handleSelector` is a
 * position a drop can land beside and never a thing that can be lifted — the
 * Lists tab's section headers (005 R508).
 */
...
/** Which way the list runs: columns across the board, routines or items down a column. */
...
export interface ListReorderOptions {
   ...
   * the details view (FR-352), and on a list item Enter and Space belong to
   * the checkbox and the text (005 FR-541) — stealing them to arm a reorder
   * would cost more than the keyboard reorder is worth. The columns' handle has
   * no other action and keeps it (FR-397).
   */
  keyboard?: boolean;
...
function pressUp(m: ReorderMachine, dropped: { current: boolean }): void {
  ...
  // The row under the finger may also be a button; the click that ends this
  // gesture must not also activate it.
