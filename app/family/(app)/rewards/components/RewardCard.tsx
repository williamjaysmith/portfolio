...
import { inkOn, mixWithWhite, profileVars, type PaletteColor } from "@/lib/family/colors";
 ...
 * therefore drawn twice, once over the whole bar in the card's ink and once
 * clipped to the fill's width in `inkOn(accent)`, so it is legible on both
 * halves however far the fill has come.
 *
 * Tokens: T037's `--fam-reward-*` names are not in `tokens.css` yet, so this
 * consumes the shipped task tokens the reference geometry is nearest to —
 * `--fam-task-card-r` (card radius), `--fam-task-emoji` (emoji), the section
 * size for the title, `--fam-task-circle-d` (bar height, 44 at the reference
 * unit), and the badge family for the pill; T037 swaps them.
 */
...
/** FR-441's busy match: one reward in one column, as a string. */
export function rewardCardKeyOf(target: Pick<RewardCardTarget, "reward" | "categoryId">): string {
...
function BarLabel({ label, layer, ink, filled }: BarLabelProps) {
  const clipped = layer === "fill" ? { clipPath: `inset(0 ${(1 - filled) * 100}% 0 0)` } : {};
  return (
    <span
      data-reward-bar-label={layer}
      style={{ color: ink, ...clipped }}
      className="absolute inset-0 grid place-items-center text-(length:--fam-fs-body) font-medium tabular-nums"
    >
      {label}
    </span>
  );
...
function RewardBar({ filled, label, accent }: RewardBarProps) {
  return (
    <span
      ...
      data-reward-bar
      data-filled={filled}
      className="fam-tint-40 relative block h-(--fam-task-circle-d) w-full overflow-hidden rounded-full"
    >
      <span
        data-reward-bar-fill
        style={{ width: `${filled * 100}%` }}
        // The fill's growth is FR-445's animated bar; `--fam-task-fade-ms` is what the
        // reduced-motion block in `tokens.css` collapses, so this needs no hook of its own.
        className="fam-tint-100 absolute inset-y-0 left-0 rounded-full transition-all duration-(--fam-task-fade-ms) ease-(--fam-task-fade-ease)"
      />
      <BarLabel label={label} layer="track" ink={inkOn(mixWithWhite(accent, 0.4))} filled={filled} />
      <BarLabel label={label} layer="fill" ink={inkOn(accent)} filled={filled} />
    </span>
  );
}
...
function RedeemButton({ reward, busy, onRedeem }: RedeemButtonProps) {
  return (
    <button
      ...
      disabled={busy}
      onClick={onRedeem}
      className="flex min-h-(--fam-touch) w-full items-center justify-center gap-(--fam-task-badge-gap) rounded-full bg-(--fam-primary-blue) px-(--fam-task-card-pad) text-(length:--fam-fs-body) font-medium text-white disabled:opacity-(--fam-past-dim)"
    >
      Redeem
...
export function RewardCard({
  ...
  return (
    <li
      ...
      className={`fam-profile flex flex-col gap-(--fam-task-badge-gap) rounded-(--fam-task-card-r) border border-(--fam-hairline) bg-(--fam-app-bg) p-(--fam-task-card-pad) text-(--fam-text-primary) shadow-sm ${
    ...
    >
      <button
        ...
        className="flex min-h-(--fam-touch) w-full flex-col items-center gap-(--fam-task-card-gap) text-center"
      >
        {reward.emoji === null ? null : (
          <span
            aria-hidden="true"
            data-reward-emoji
            className="text-(length:--fam-task-emoji) leading-none"
          >
            {reward.emoji}
          </span>
        )}
        <span
          data-reward-title
          className="max-w-full truncate font-(family-name:--fam-font-serif) text-(length:--fam-fs-section)"
        >
          {reward.name}
        </span>
