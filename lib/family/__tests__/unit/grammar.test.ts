...
describe("parseRule", () => {
  ...
  it("parses a minimal endless daily rule", () => {
    expect(parseRule("FREQ=DAILY;INTERVAL=1")).toEqual({ freq: "DAILY", until: null });
  });
  ...
  describe("refuses everything outside the closed grammar", () => {
    it.each([
      ...
      ["INTERVAL other than 1", "FREQ=DAILY;INTERVAL=2", /INTERVAL/],
...
describe("emitRule", () => {
  ...
  it("always writes INTERVAL=1 (a stored rule is self-describing)", () => {
    expect(emitRule({ freq: "DAILY", until: null })).toBe("FREQ=DAILY;INTERVAL=1");
  });

  it("emits a plain-date UNTIL for an all-day series", () => {
    expect(emitRule({ freq: "DAILY", until: { kind: "date", date: "2026-12-17" } })).toBe(
  ...
  it("emits a timed UNTIL as the zoned end-of-day UTC instant, not T235959Z on the date", () => {
    ...
    expect(emitRule({ freq: "DAILY", until: { kind: "instant", ms: CHICAGO_EOD_MS } })).toBe(
  ...
  it("emits WKST before BYDAY on weekly rules and omits it when absent", () => {
    expect(
      emitRule({ freq: "WEEKLY", until: null, wkst: "MO", byDay: ["WE"] }),
    ).toBe("FREQ=WEEKLY;INTERVAL=1;WKST=MO;BYDAY=WE");
    expect(
      emitRule({ freq: "WEEKLY", until: null, wkst: null, byDay: ["WE"] }),
    ).toBe("FREQ=WEEKLY;INTERVAL=1;BYDAY=WE");
  });

  it("preserves BYDAY order as given", () => {
    expect(
      emitRule({ freq: "WEEKLY", until: null, wkst: "SU", byDay: ["FR", "MO"] }),
    ).toBe("FREQ=WEEKLY;INTERVAL=1;WKST=SU;BYDAY=FR,MO");
  });

  it("emits BYMONTHDAY on monthly rules", () => {
    expect(emitRule({ freq: "MONTHLY", until: null, byMonthDay: 31 })).toBe(
  ...
  describe("refuses rule objects the grammar cannot express", () => {
    it.each<[string, RecurrenceRule, RegExp]>([
      ["an empty BYDAY", { freq: "WEEKLY", until: null, wkst: "SU", byDay: [] }, /weekday/],
      [
        "a duplicate weekday",
        { freq: "WEEKLY", until: null, wkst: "SU", byDay: ["MO", "MO"] },
        /duplicate/,
      ],
      ["byMonthDay 0", { freq: "MONTHLY", until: null, byMonthDay: 0 }, /1–31/],
      ["byMonthDay 32", { freq: "MONTHLY", until: null, byMonthDay: 32 }, /1–31/],
      ["a fractional byMonthDay", { freq: "MONTHLY", until: null, byMonthDay: 1.5 }, /1–31/],
      [
        "an until date not in YYYY-MM-DD form",
        { freq: "DAILY", until: { kind: "date", date: "20261217" } },
        /YYYY-MM-DD/,
      ],
      [
        "an impossible until date",
        { freq: "DAILY", until: { kind: "date", date: "2026-02-30" } },
        /real/,
      ],
      [
        "an until instant with sub-second precision (cannot round-trip)",
        { freq: "DAILY", until: { kind: "instant", ms: CHICAGO_EOD_MS + 500 } },
        /second/,
      ],
      [
        "a non-finite until instant",
        { freq: "DAILY", until: { kind: "instant", ms: Number.NaN } },
        /second|instant/,
      ],
...
describe("round-trips", () => {
  ...
  it("parse(emit(rule)) preserves every field", () => {
    const rules: RecurrenceRule[] = [
      ...
      { freq: "DAILY", until: { kind: "date", date: "2026-11-01" } },
      { freq: "WEEKLY", until: null, wkst: null, byDay: ["TU"] },
      {
        freq: "WEEKLY",
      ...
      { freq: "MONTHLY", until: { kind: "date", date: "2027-06-30" }, byMonthDay: 29 },
    ];
