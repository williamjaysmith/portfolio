...
DEAD_CODE_BASELINE=".specify/local/fallow-baselines/dead-code-baseline.json"
DUPES_BASELINE=".specify/local/fallow-baselines/dupes-baseline.json"
BASELINE_ARGS=()
if [ -f "$DEAD_CODE_BASELINE" ]; then
  BASELINE_ARGS+=(--dead-code-baseline "$DEAD_CODE_BASELINE")
fi
if [ -f "$DUPES_BASELINE" ]; then
  BASELINE_ARGS+=(--dupes-baseline "$DUPES_BASELINE")
fi
if [ "${#BASELINE_ARGS[@]}" -gt 0 ]; then
  BASELINE_ARGS+=(--fail-on-regression --tolerance 0)
fi

if "${RUNNER[@]}" audit --format json --quiet --explain "${BASELINE_ARGS[@]}" >"$TMP_JSON" 2>"$TMP_ERR"; then
  STATUS=0
else
  STATUS=$?
fi
