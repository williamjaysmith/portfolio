...
A parent opens `willsmith.dev/family` and signs in with Google. Their address is on the household allowlist, so the app opens on the Calendar tab. Anyone else — a stranger who finds the URL, or a Google account that isn't on the list — is refused and never sees household data.

**Why this priority**: This is a real family's schedule, including a child's. Without it nothing else may ship. Constitution §VII.

**Independent Test**: Sign in as an allowlisted account and reach the shell; sign in as a non-allowlisted Google account and be refused; request the page signed-out and be redirected. Query the database directly as a non-member and get zero rows.
...
2. **Given** an allowlisted parent, **When** they complete Google sign-in, **Then** they land on the Calendar tab of the shell.
3. **Given** a Google account not on the allowlist, **When** they complete Google sign-in, **Then** they see an "unrecognised account" message, are signed out, and no household data is returned.
4. **Given** a signed-in member, **When** a request is made for another household's data, **Then** the store returns nothing regardless of what the interface requested.
5. **Given** an allowlisted parent on the wall tablet, **When** they return days later without re-authenticating, **Then** their session is still valid.
6. **Given** any visitor, **When** a search engine crawls the site, **Then** `/family` is excluded from indexing.
...
- **FR-002**: The system MUST offer Google as the sign-in method.
...
- **FR-004**: The system MUST NOT provide any public sign-up path.
...
- **Household Member** — the link between an authenticated account and the household; the allowlist. Determines whether someone may see anything at all.
...
- **SC-001**: A person who is not on the household allowlist cannot reach household content by any route — signed out, signed in with another account, or requesting data directly. Verified by an explicit test for each of the three paths.
...
- **Google is the only sign-in method** for this phase, chosen for one-tap use on a tablet. Adding email magic links later requires no data change.
...
- A Google OAuth client registered and configured as a Supabase auth provider.
