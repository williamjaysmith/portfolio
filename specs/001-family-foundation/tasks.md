...
**Amended**: 2026-09-02 — sign-in changed from Google to a single shared household password (spec FR-002, research R13). The tasks that built the Google surface are marked **`[~]` superseded** rather than deleted, so the list stays an honest record of what was built and why it was unbuilt; the replacement work is T066–T072.
...
- `[X]` done · `[ ]` outstanding · `[~]` **superseded** — was built, then removed by a later decision; kept for the record with a pointer to what replaced it.
...
- [X] T004 [P] `supabase/config.toml` (init already done): `api.schemas` += `family`; the Portfolio port block `55320–55329` (another project's stack owns `54321–54329` on this machine — never stop it); `auth.site_url`/`additional_redirect_urls` for `localhost:3000`; email provider on; `supabase/seed.sql` comment-only placeholder; gitignore `supabase/.temp`. **`supabase link` is an operator step** (quickstart §4) — not part of this task. **Amended 2026-09-02 (T068, outstanding):** the `[auth.external.google]` block and its `env(...)` client id/secret go; email+password is the only provider locally and hosted, and sign-ups stay enabled locally so `db reset` + seed always gets back in
...
- [X] T012 Migration `supabase/migrations/007_seed.sql` — the household (`00000000-0000-4000-8000-000000000001`, "Our Family") + its settings row **only**; no emails, no names, no PINs. Plus `scripts/family-seed.mjs` (ESM, admin client, idempotent upserts, prints what it did): reads `FAMILY_SEED_PARENT_EMAILS` + optional `FAMILY_SEED_PROFILES` from `.env.local`; `--local` targets the local stack, creates dev user `dev@family.local`, allowlists it, seeds fixture profiles; refuses a non-local URL without `--yes`. **Amended 2026-09-02 (T068):** the script now *creates* the one shared household account (`email_confirm: true`, no mail) as well as allowlisting it, from `FAMILY_ACCOUNT_EMAIL` + `FAMILY_ACCOUNT_PASSWORD` (hosted) or `dev@family.local` + `FAMILY_DEV_PASSWORD` (`--local`), and never prints the password. `FAMILY_SEED_PARENT_EMAILS` survives as *optional extra* allowlist addresses rather than the per-parent list it was
...
**Independent Test**: quickstart "SC-001" — three paths (signed out, wrong password, direct REST) all fail; the household password lands in the shell.

- [X] T021 [US1] `proxy.ts` at repo root — matcher `['/family/:path*']`; always refreshes the session via `updateSession` (FR-006); adds `X-Robots-Tag: noindex, nofollow` to every `/family` response; redirects to `/family/sign-in` only for protected paths — public: `/family/sign-in`, `/family/auth/*`, `/family/not-authorized`, `/family/manifest.webmanifest`, `/family/icons/*`, `/family/avatars/*`; redirect responses copy the refreshed cookies (`getAll().forEach(c => res.cookies.set(c))`); a comment stating it is NOT the authorization boundary (research R1). **Amended 2026-09-02 (T067):** `/family/auth/*` leaves the public-path list along with the callback route it existed for
- [~] T022 [P] [US1] ~~`app/family/(auth)/sign-in/page.tsx` + `GoogleSignInButton.tsx` (client) — copy "Family calendar" / "Continue with Google" / "Only household accounts can sign in."; `signInWithOAuth` with `redirectTo` `${origin}/family/auth/callback`; brand-styled with tokens~~ — **SUPERSEDED 2026-09-02 by T066.** The household wanted no Google account; `GoogleSignInButton.tsx` is deleted and the page becomes one password field. The title copy and the token styling survive into T066
- [~] T060 [P] [US1] ~~`app/family/(auth)/sign-in/DevSignIn.tsx` (client) — email + password form rendered **only when** `NODE_ENV !== 'production'` **and** `NEXT_PUBLIC_SUPABASE_URL` matches `^https?://(127\.0\.0\.1|localhost)(:|/|$)`~~ — **SUPERSEDED 2026-09-02 by T066.** The dev form existed only because Google needed operator setup before anything could be seen locally; with a password field that works identically in both environments there is nothing for it to do, and a production-excluded second sign-in path is one less thing to keep safe. **The `dev:local` npm script is NOT superseded** — it still runs `next dev --turbopack` with the local Supabase URL and keys inlined
- [~] T023 [P] [US1] ~~`app/family/(auth)/auth/callback/route.ts` — `?error=` from the auth hook → `/family/not-authorized`; `exchangeCodeForSession`, then `rpc('claim_membership')` as the user; `null` → `signOut()` + redirect `/family/not-authorized`; otherwise redirect `/family/calendar`~~ — **SUPERSEDED 2026-09-02 by T067.** No OAuth code to exchange, so no callback. Membership binds on the first page load instead: `requireMember()` already falls back to `claim_membership()` (contracts, guards table). The route and its `/family/auth/*` public path in `proxy.ts` are deleted
- [X] T024 [P] [US1] `app/family/(auth)/not-authorized/page.tsx` — allowlist rejection with no household data. **Amended 2026-09-02 (T069, outstanding):** the copy still says "That Google account isn't part of this household." + "Try another account"; with one shared account it becomes "That account isn't part of this household." + "Back to sign-in" (signs out). The page is now a backstop rather than a routine path — reachable only when an account exists that the allowlist does not know
- [X] T061 [P] [US1] `lib/family/actions/auth.ts` — `signOut()`: `clearActor()` + `supabase.auth.signOut()` then `redirect('/family/sign-in')` outside `try/catch`; used by not-authorized and Settings → Account. **Amended 2026-09-02 (T067):** `signIn(password)` joins it in the same file, and the Account section stops rendering "Signed in as {email}" — with one shared account the address names nobody and must not reach the client (FR-002)
...
- [ ] T027 **[BLOCKED ON OPERATOR]** [US1] Manual quickstart SC-001 run on the hosted project — record the three outcomes (signed out, wrong password, direct REST) in the PR/commit body. **blocked-on-operator** (schema exposure, the household account seed, sign-ups off, hook — quickstart §4)
...
## Phase 9: Auth change — one shared household password (2026-09-02)

**Why**: spec FR-002 / research R13. The household is two adults who wanted neither a Google account nor an email service. `/family` gets one Supabase account; the sign-in screen becomes a single password field; who is *acting* stays where it always was — the punch-in PINs. Supersedes T022, T023, T060, and amends T004, T012, T024, T040, T061.

**Independent Test**: quickstart SC-001 (a) signed out → redirect, (b) wrong password → one message and no session, (c) direct REST with the publishable key → `401`/`42501`; and the household password reaches `/family/calendar` with the allowlist row claimed on that first load.

**Status 2026-09-02**: T066 and T067 landed (form, action, callback and dev form deleted, `proxy.ts` updated). Outstanding: T068's `.env.example` and `supabase/config.toml` half, T069 (copy sweep), T070 (tests + the four gates). T071 is the operator's.

- [X] T066 [US1] Replace the sign-in surface — `app/family/(auth)/sign-in/page.tsx` + `SignInForm.tsx` (client): **one** password field (`type="password"`, `autoComplete="current-password"`, `required`, 44 px minimum target), a submit button, and a permanently rendered `role="alert"` line carrying the single non-specific failure message; title "Family calendar", footer "Only household accounts can sign in.", token styling from T022; the page redirects an already-signed-in visitor to `/family/calendar`. No email field, no provider button, no sign-up link, and **no account address anywhere in the markup** (FR-002). Deleted `GoogleSignInButton.tsx` and `DevSignIn.tsx`
- [X] T067 [US1] `signIn` in `lib/family/actions/auth.ts` — a `useActionState` reducer `(previousState, formData) => Promise<ActionResult<null>>`, so the form works before hydration; reads `FAMILY_ACCOUNT_EMAIL` via `familyAccountEmail()` in `lib/family/env.ts`, short-circuits an empty password, calls `supabase.auth.signInWithPassword`, clears any stale actor cookie, then `redirect('/family/calendar')` outside `try/catch`. Rejected credentials → `NOT_AUTHENTICATED` + "That password isn't right." for every case; a missing env var or an unreachable Supabase → `UNAVAILABLE`, logged as a string. **Supabase validates the password — the app never holds or compares one.** Deleted `app/family/(auth)/auth/callback/route.ts` and dropped `/family/auth/*` from the `proxy.ts` public-path list (membership binds via `requireMember()`'s `claim_membership()` fallback)
- [x] T068 [US1] Environment + seed — **seed script done; `.env.example` and `supabase/config.toml` outstanding as of 2026-09-02.** `FAMILY_ACCOUNT_EMAIL` (no `NEXT_PUBLIC_` prefix, ever) and `FAMILY_ACCOUNT_PASSWORD` documented in `.env.example` (which still describes `FAMILY_SEED_PARENT_EMAILS` as "Google account emails") and quickstart §2; `scripts/family-seed.mjs` creates the single household account (`email_confirm: true`, so nothing is mailed) and its `household_users` allowlist row, idempotently, in both `--local` (`dev@family.local` + `FAMILY_DEV_PASSWORD`) and `--yes` modes, printing the address and never the password; `FAMILY_SEED_PARENT_EMAILS` demoted to optional extra allowlist addresses; `supabase/config.toml` drops the `[auth.external.google]` block. **Also set `FAMILY_ACCOUNT_EMAIL` in the Vercel project** — without it, deployed sign-in fails with `UNAVAILABLE`
- [x] T069 [P] [US1] Copy sweep — `not-authorized/page.tsx` still says "That Google account isn't part of this household." / "Try another account" / "Ask a parent to add your address": it becomes "That account isn't part of this household." + "Back to sign-in", and its doc comment ("proved nothing except that they have a Google account") goes with it. Settings → Account: "Signed in to the household account" replacing "Signed in as {email}", with `userEmail` removed from `FamilyInitialData` and its provider plumbing so the address never reaches the client
- [x] T070 [US1] Tests — `app/family/__tests__/sign-in.test.tsx` covers the form (exactly one field; it is a password field; no address in the output) and the action's single message for wrong password / empty field / unknown account, plus `UNAVAILABLE` when `FAMILY_ACCOUNT_EMAIL` is unset; `callback-route.test.ts` deleted and `proxy.test.ts` updated for the dropped `/family/auth/*` path. Unit project, no network. Confirm the full suite and the four gates are green before this is checked
- [x] T071 **[DONE 2026-09-02 in the auth rework]** [P] Hosted auth settings in the quickstart §4 order — schema pushed and exposed, `FAMILY_ACCOUNT_*` set, **seed the account first**, then Email provider **on**, "Allow new users to sign up" **off**, anonymous sign-ins **off**, Before-User-Created hook **on**. Closing sign-ups before the account exists can refuse the household's own account. **blocked-on-operator** (Dashboard)
- [X] T072 Documentation sync for this change — `spec.md` (FR-002, FR-004 enforcement, US1 narrative + scenarios, SC-001, the dated decision under Assumptions, Dependencies, the forgotten-password edge case), `contracts/server-actions.md` (`signIn`, callback removed, the indistinguishable failure), `plan.md` (structure tree, phasing, risks, constitution re-check, progress), `research.md` (R13 with what was rejected), `quickstart.md` (operator steps, env table, troubleshooting), and this file. `data-model.md` deliberately untouched — the schema does not change
...
Auth change (T066–T072, 2026-09-02) re-opens US1 only:
  T066 (form) ─┬─► T070 (tests)
  T067 (action)┘        T068 (env + seed) ─► T071 (operator, hosted)
  T069 (copy) independent · T072 (docs) done
```

US1→US2→US3 are genuinely sequential (each consumes the previous story's mechanism). US5 is independent after US1. Within every phase, `[P]` tasks can run concurrently. Tasks marked **blocked-on-operator** (T013's live push, T027, T050, T071) do not block anything that runs against the local stack.
...
- **US1**: T024, T061, T062 together after T021 (T022, T023, T060 superseded)
- **Auth change**: T066 + T067 in one stream (form and action are one round trip), T068 and T069 alongside; T070 after T066/T067
- **US2**: T030, T031 while T028, T029, T059 land
- **US3**: T035, T064, T038, T039 together before T040–T041
- **US4**: T043, T044, T065 together
- **MVP scope**: Phases 1–3 only (T001–T027 + T055–T057, T061, T062, and T066–T068 in place of the superseded T022/T023/T060) — sign-in, allowlist, database-enforced privacy, placeholder shell. Everything after is feature, not foundation.

## Implementation waves (2026-08-31)

The build brief runs the foundation as parallel waves with disjoint file ownership: **A** database & seed (T004, T006–T013, T055, T056) · **B1/B2** pure lib (T014–T017, T057, T038's `avatars.ts`) · **C** server plumbing (T003, T018, T021, `next.config.ts`) · **D1/D2** tokens, layout, manifest, icons, avatar artwork (T019, T020, T038, T049) · **E** test infrastructure (T005, T026, T034, T042, T062, T063) · **F** this documentation sync. Wave 2 — auth surface (T022–T025, T060, T061; T022/T023/T060 superseded on 2026-09-02 and rebuilt as T066–T067), actions (T028, T029, T036, T037, T040's action, T059, T064, T035), shell chrome (T043–T046, T065), punch-in UX (T030–T033), settings (T039–T041) — starts once wave 1 typechecks.

## Format validation

72 tasks (65 + T066–T072 for the 2026-09-02 auth change), of which 3 are superseded (`[~]` T022, T023, T060) and stay listed; every task has a checkbox + ID + file path; `[P]` only where files are disjoint; story labels on all story-phase tasks and only there. ✓
