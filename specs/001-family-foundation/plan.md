...
**Scale/Scope**: One household, 3 profiles, 2 authenticated accounts, ~5 concurrent devices. Roughly 25 source files and 7 migrations.
...
|---|---|---|
| **I. Sub-apps are self-contained** | PASS | Everything lives under `app/family/**` and `lib/family/**`. No existing portfolio file is modified except `.gitignore`/deps. Nothing is extracted for reuse — there is no second consumer. |
...
- **`set_pin` requires only membership, not a punched-in parent.** A deliberate narrowing to satisfy FR-018/SC-010, without which a PIN-less household is permanently read-only. The authority is still proven family membership. Re-check: **PASS**.
...
app/family/
├── layout.tsx                            # fonts, tokens, metadata, noindex
...
└── (app)/
    ├── layout.tsx                        # FamilyProvider + shell chrome
    ├── page.tsx                          # redirect → /family/calendar
    ├── calendar|tasks|rewards|meals|lists/page.tsx   # placeholders
    ├── settings/page.tsx                 # profiles, labels, PINs, preferences
    └── components/
        ├── Sidebar.tsx  BottomNav.tsx  TopBar.tsx
        ├── ProfileChipRow.tsx  ProfileChip.tsx
        ├── Fab.tsx  PunchInSheet.tsx  ActorBadge.tsx
        └── ProfileForm.tsx  ColorPicker.tsx  AvatarPicker.tsx

lib/family/
├── types.ts                              # shared domain types
├── colors.ts                             # 20-colour palette + tint derivation
├── permissions.ts                        # pure authorization rules
├── actor-token.ts                        # sign/verify the actor JWT
├── actor.ts                              # cookie read/write (server-only)
├── guards.ts                             # requireMember/requireActor/requireParent
├── queries.ts                            # RLS-backed reads
├── validation.ts                         # Zod schemas
├── supabase/{client,server,admin,proxy}.ts
├── actions/{punch-in,categories,avatars,settings}.ts
└── __tests__/                            # unit + policy suites

supabase/migrations/001–007_*.sql
public/family/icons/                      # PWA icons, illustrated avatars
```
...
| 2 | Migrations 001–005, applied via CLI | `supabase db push`; tables visible; policy suite red (nothing to read yet) |
...
| 9 | Manifest + icons; seed migration 007 | Installs to the iPad home screen and launches full-screen |
