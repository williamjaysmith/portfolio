...
| **VIII. Fidelity is specified** | PASS | All 45 FRs are tagged; the two `[V-photo]` corrections Phase 3 recorded are inherited; the estimated geometry (chip, bar, modal, sprites) stays `[ESTIMATED]`/`[OURS]` in the token layer. The one place design narrowed a spec sentence is stated in the open: FR-405's "the household-local day of the completion" is the **occurrence's** date for a dated chore (so the pill on that day shows it) and the resolution's day for an anytime chore — the spec's own edge case says the same, and the trigger's `coalesce` is that sentence. |
