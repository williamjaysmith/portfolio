...
| **SC-414** the rain fires on completion by a tap | Not on a skip, not by hiding, again after undo | Leave one outstanding; skip it → nothing; unskip, then hide it with a filter → nothing (the list is not complete); show it and complete it → the rain plays; un-tick and re-tick → plays again; two quick taps on the last two → once, on the second |
