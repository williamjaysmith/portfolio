...
- [x] T061 **Hosted, in this order** (Hard ordering, R605): `supabase db push --linked` (030–033) → quickstart §4 steps 2–3 (no `anon` grant; both functions `service_role` only; four tables published at replica identity `d`) → `npm run family:seed -- --yes` → exactly four mealtimes → merge to `main`, push, deploy → SC-614 and SC-615 on the live site
