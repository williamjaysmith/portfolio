...
- [ ] T062 [US4] Labels section in `app/family/(app)/components/FilterSheet.tsx` — same 44 pt checkbox-row pattern with a colour swatch in place of the avatar, same `setHidden`/`showAll` (Show all clears both kinds — FR-264, US4-9); **the storage layer does not change** — Phase 1's `useDeviceVisibility` already stores generic category ids per device with the in-memory fallback (FR-266, R212); RTL test for the section
