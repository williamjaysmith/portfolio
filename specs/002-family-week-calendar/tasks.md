...
- [ ] T026 [P] [US1] Grid tokens in `app/family/tokens.css` — hour row height, gutter width, now-line (`#F66951`, 2 pt, ~14 pt dot — photo-estimated values commented as such, FR-208), today-circle (~44 pt `[ESTIMATED]`, FR-209), stripe geometry (≈45°, ≈40 pt, tagged — FR-212/Assumption 17), weekend shade + past-dim values (FR-215), neutral fill + border (FR-213)
...
- [ ] T030 [US1] `app/family/(app)/calendar/components/WeekGrid.tsx` + `DayColumn.tsx` — one column per day over the hour ruler (FR-201), weekend column shading, past-event dimming at minute granularity (FR-215), vertical hour scroll (FR-280), per-day overflow scroll so all twelve of a full day are reachable (FR-216, SC-210)
...
- [ ] T032 [P] [US1] `app/family/(app)/calendar/components/NowLine.tsx` — the 2 pt orange bar + gutter dot in today's column only, drawn above blocks, repositioned each minute from the shared clock store, no reload (FR-208, US1-3/4)
